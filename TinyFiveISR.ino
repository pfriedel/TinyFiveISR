...
const uint8_t led_dir[15] = {
  ( 1<<1 | 1<<0 ), // 1 r (00010 | 00001 = 00011) 
  ( 1<<0 | 1<<4 ), // 2 r (00001 | 10000 = 10001)
  ( 1<<4 | 1<<3 ), // 3 r (10000 | 01000 = 11000)
  ( 1<<3 | 1<<2 ), // 4 r (01000 | 00100 = 01100)
  ( 1<<2 | 1<<1 ), // 5 r (00100 | 00010 = 00110) 

  ( 1<<1 | 1<<2 ), // 1 g (00010 | 00100 = 00110)
  ( 1<<0 | 1<<1 ), // 2 g (00001 | 00010 = 00011)
  ( 1<<4 | 1<<0 ), // 3 g (10000 | 00001 = 10001)
  ( 1<<3 | 1<<4 ), // 4 g (01000 | 10000 = 11000)
  ( 1<<2 | 1<<3 ), // 5 g (00100 | 01000 = 01100)

  ( 1<<1 | 1<<3 ), // 1 b (00010 | 01000 = 01010)
  ( 1<<0 | 1<<2 ), // 2 b (00001 | 00100 = 00101) 
  ( 1<<4 | 1<<1 ), // 3 b (10000 | 00010 = 10010)
  ( 1<<3 | 1<<0 ), // 4 b (01000 | 00001 = 01001)
  ( 1<<2 | 1<<4 ), // 5 b (00100 | 10000 = 10100)
};

//PORTB output config for each LED
//PORTB=1 where DDRB=1 = high
//PORTB=0 where DDRB=0 = low
...
//    for(led = 0; led<5; led++) {
//      for(b = 0; b < max_brite; b++) {
//	if(b < led_grid[led]) { DDRB |= led_dir[led]; }
//	else { DDRB = 0; } // Really I just want to set
//	if(b < led_grid[led+5]) { DDRB |= led_dir[led+5]; }
//	else { DDRB = 0; }
//	if(b < led_grid[led+10]) { DDRB |= led_dir[led+10]; }
//	else { DDRB = 0; }
//	PORTB = led_out[led];
//      }
//      DDRB = 0;
